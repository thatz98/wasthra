...
class User_Model extends Model{
    ...
    public function getUser($id,$type){
        ...
        if($type=='customer'){
            ...
            return $this->db->query("SELECT admin.user_id,admin.first_name,admin.last_name,admin.gender,admin.email,admin.contact_no,login.user_status,login.user_type FROM admin INNER JOIN login ON admin.login_id=login.login_id WHERE admin.user_id='$id';");
        }else if($type=='owner'){
            return $this->db->query("SELECT owner.user_id,owner.first_name,owner.last_name,owner.gender,owner.email,owner.contact_no,login.user_status,login.user_type FROM owner INNER JOIN login ON owner.login_id=login.login_id WHERE owner.user_id='$id';");
        }else if($type=='delivery_staff'){
            return $this->db->query("SELECT delivery_staff.user_id,delivery_staff.first_name,delivery_staff.last_name,delivery_staff.gender,delivery_staff.email,delivery_staff.contact_no,login.user_status,login.user_type FROM delivery_staff INNER JOIN login ON delivery_staff.login_id=login.login_id WHERE delivery_staff.user_id='$id';");
        }

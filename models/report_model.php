...
class Report_Model extends Model {
    ...
    function getOrderItems($id) {
        ...
        INNER JOIN product_images ON order_item.product_id=product_images.product_id WHERE order_item.order_id=:id GROUP BY order_item.product_id ",array('id'=>$id));
    }
    ...
    function getOrderInfo($id) {
        ...
        INNER JOIN delivery_address ON checkout.address_id=delivery_address.address_id  
         WHERE orders.order_id=id GROUP BY orders.order_id ",array('id'=>$id));
    }

    function getInventoryData(){
        return $this->db->runQuery("SELECT inventory.product_id,inventory.qty,inventory.reorder_qty,inventory.reorder_level,inventory.color,inventory.size,product.product_name FROM inventory
